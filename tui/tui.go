...
func (m *MainModel) initForm(mode FormMode, server *config.Server) {
	m.formMode = mode
	m.formInputs = make([]textinput.Model, 8)
	m.formActiveField = 0
	...
	m.formInputs[4].Placeholder = "password (or passphrase if using key)"
	m.formInputs[4].EchoMode = textinput.EchoPassword
	m.formInputs[5].Placeholder = "/path/to/my/project"
	m.formInputs[6].Placeholder = "/home/user/.ssh/id_rsa (optional)"
	m.formInputs[7].Placeholder = "n (y/n to use SSH Agent)"
	...
	if mode == ModeEdit && server != nil {
		...
		m.formInputs[6].SetValue(server.PrivateKeyPath)
		if server.UseSSHAgent {
			m.formInputs[7].SetValue("y")
		} else {
			m.formInputs[7].SetValue("n")
		}
	} else {
...
func (m MainModel) updateServerForm(msg tea.Msg) (tea.Model, tea.Cmd) {
	...
	case tea.KeyMsg:
		...
		case "enter":
			...
			privateKeyPath := strings.TrimSpace(m.formInputs[6].Value())
			useSSHAgentVal := strings.ToLower(strings.TrimSpace(m.formInputs[7].Value()))
			useSSHAgent := useSSHAgentVal == "y" || useSSHAgentVal == "yes" || useSSHAgentVal == "true"

			if alias == "" || host == "" || user == "" {
				m.errorMsg = "Alias, Host, and Username are required fields."
				return m, nil
			}

			if !useSSHAgent && privateKeyPath == "" && password == "" {
				m.errorMsg = "Password is required when not using SSH Agent or Private Key."
				return m, nil
			}
